  ...
  <main class="shop-page">
    ...
    <div class="shop-toolbar">
      <button class="filters-toggle" @click="filtersOpen = !filtersOpen">
        Filtros</button
      ><label
        >Ordenar<select v-model="filters.sort" @change="load">
    ...
    <p v-else-if="error" class="error-state">
      No hemos podido cargar la tienda. Inténtalo de nuevo.
    </p>
...
const isSearch = computed(() => !!term.value);
...
async function load() {
  ...
  const result = isSearch.value
    ? await searchProducts(term.value, filters.value)
    : await getProducts(filters.value);
  if (result.error) {
    console.error(result.error);
    error.value = true;
  } else products.value = result.data || [];
  loading.value = false;
  document.title = isSearch.value
    ? `Buscar: ${term.value} | Tienda`
    : "Tienda | Rincón de la esperanza";
}
onMounted(async () => {
