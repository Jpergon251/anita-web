...
const PUBLIC_FIELDS =
  "id, name, slug, description, price, stock, image_url, is_active, created_at, category_id, categories(id, name, slug)";
...
function queryWithFilters(query, filters = {}) {
  ...
  const orders = {
    priceAsc: ["price", { ascending: true }],
    priceDesc: ["price", { ascending: false }],
    nameAsc: ["name", { ascending: true }],
    nameDesc: ["name", { ascending: false }],
...
export async function getProductById(id) {
  return supabase.from("products").select(PUBLIC_FIELDS).eq("id", id).single();
}
